...
- Part of **Protocolization** for World App: all signature-requiring flows are crafted client-side via open-source Bedrock, minimizing trust in the TFH backend.
...
| 1–5    | 5 B  | `magic`       | Constant `0x626472636b` (`"bdrck"`). Distinguishes Bedrock-built operations; collision probability ≈ 9.1×10⁻¹³. |
...
| 17–23  | 7 B  | `random`      | 56 bits of entropy; collision risk for 1M tx in a bucket ≈ 0.0007%.                                             |
...
- On every app launch we call `/world-chain`, which now returns `requiresUpgrade`. If `true`, the UI blocks until the user runs the combined Safe upgrade + 4337 module enablement flow.
- Wallet upgrades also prepare the ground for Project Safeguard and future Safe import functionality.
